...
function mise_en_cache($page,$content){
	...
	$fichier = fopen($path.'cache/'.$page.'.cache', 'w+');
...
function affiche_cache($page){
	...
	$content = file_get_contents($path.'cache/'.$page.'.cache');    
	return $content;
}

//fonction de génération du retour utilisateur en cas d'erreur ou d'information
function erreur_info($type,$titre=null,$message='erreur_interne',$cible,$tempo=5){
	global $path,$nom_site;
	//récupération d'un affichage type
	$structure=file_get_contents($path.'page/information.php');
	//mise en place du type
	$structure=str_replace('||type||', $type, $structure);
	//changement du titre
	if(($titre=='')){
		$titre=$nom_site;
	}
	//mise en place du titre
	$structure=str_replace('||title||', _($titre), $structure);
	//mise en place de la temporisation
	$structure=str_replace('||tempo||', $tempo, $structure);
	//mise en place du message de redirection
	$structure=str_replace('||mess_redir||', _('mess_redir'), $structure);
	//mise en place du message clicable de redirection
	$structure=str_replace('||mess_clic_redir||', _('mess_clic_redir'), $structure);
	//mise en place de la cible
	$structure=str_replace('||cible||', $cible, $structure);
	//mise en place du message
	$structure=str_replace('||message||', _($message), $structure);
